...
DJ::DJ(QWidget *parent) : QDialog(parent)
{
           ...
           //theme->setSource(QUrl("qrc:/audio/coin.wav"));

           this->theme = new QSoundEffect;//创建对象
           theme->setSource(QUrl("qrc:/audio/finalfrog.wav"));//添加资源
           theme->setLoopCount(QSoundEffect::Infinite);//设置循环次数int；  QSoundEffect::Infinite 枚举值 无限循环
           theme->setVolume(.20f);
