...
void MainWindow::loginwindow()
{
    ...
    QPixmap loginbackgroundimage("://picture/greenbg.png");//绿色的背景图
    ...
    QPixmap icon1("://picture/frogrun.png");
    ...
    scene->addItem(iconpic1);        //设置登录界面的青蛙

    QPixmap mainlogo("://picture/frgologo.png");
    ...
    //创建开始按钮
    startbutton = new QPushButton(ui->graphicsView);
    ...
    settingbutton = new QPushButton(ui->graphicsView);
    //创建选项按钮
    ...
    exitbutton = new QPushButton(ui->graphicsView);
    //创建退出按钮
...
unsigned int i=0;
void MainWindow::nextframe()//这里是在干嘛呢？实现被按的时候按钮变大？
{
...
void MainWindow::changecolor()//按钮被按到后变色操作
{
...
void MainWindow::gamebegin()
{
    ...
    connect(&update_timer,&QTimer::timeout,[=](){
        ...
        cat[0].cat_auto_down();//青蛙的自动下落初始
        cat[1].cat_auto_down();//青蛙的自动下落初始
        cat_any_down();//青蛙自动下落的补充
        cat_float();
        cat_moveground_down();
        cat[0].cat_move();//反复调用青蛙是否移动的检测函数
        cat[1].cat_move();//反复调用青蛙是否移动的检测函数
    });
        cat[1].cat_y+=BIAS+10;
        cat[1].iscat2=true;//代表是二号猫
        cat[1].cat_jump_height=cat[1].cat_y-270;
}

void MainWindow::setCoin()//放置金币
{
...
void MainWindow::paintEvent(QPaintEvent *)
{
    if(gamemode == 1)//单人模式
    {
        ...
        QPixmap backgroundImage2("://picture/secondbg.png");
